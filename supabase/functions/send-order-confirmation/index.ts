...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const discountRow = Number(order.discount_amount || 0) > 0
      ? `<tr><td style="padding:10px 8px;color:#86d39e">DESCUENTO 10% · ${order.discount_code}</td><td style="padding:10px 8px;text-align:right;color:#86d39e">- S/ ${Number(order.discount_amount).toFixed(2)}</td></tr>`
      : ''
    const html = `<div style="background:#080808;color:#f8f5ec;padding:38px;font-family:Arial,sans-serif;max-width:620px;margin:auto"><p style="color:#d9b632;letter-spacing:2px;font-size:12px">LYNX BOUTIQUE</p><h1 style="margin:8px 0 20px">PEDIDO CONFIRMADO</h1><p>Hola ${order.customer_name}, confirmamos tu pedido <strong style="color:#d9b632">${order.order_code}</strong>.</p><table style="width:100%;border-collapse:collapse;margin:24px 0">${rows}${discountRow}<tr><td style="padding:12px 8px;font-weight:bold">TOTAL</td><td style="padding:12px 8px;text-align:right;font-weight:bold;color:#d9b632">S/ ${Number(order.total).toFixed(2)}</td></tr></table><p>Entrega: ${order.shipping_method === 'shalom' ? 'Shalom · flete pagado en agencia' : 'Motorizado en Lima · separación S/ 50 y saldo por Yape, Plin o transferencia. No aceptamos efectivo'}.</p><p style="color:#aaa;font-size:12px;line-height:1.6">Guarda este correo. Te avisaremos por WhatsApp cuando tu pedido cambie de estado.</p></div>`
