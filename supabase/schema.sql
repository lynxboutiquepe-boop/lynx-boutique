...
-- Reseñas reales de clientes, publicadas solo tras moderación administrativa.
...
create or replace function public.submit_customer_review(p_rating smallint, p_comment text)
...
begin
  ...
  select full_name, email_verified into v_name, v_verified from public.customer_profiles where user_id = auth.uid();
  ...
  on conflict (user_id) do update set author_name = excluded.author_name, rating = excluded.rating, comment = excluded.comment, status = 'pending', updated_at = now()
